...
router.get(
  ...
  asyncHandler(async function (req: ReposAppRequest, res, next) {
    ...
    try {
      ...
      const title =
        state === true
          ? 'Please enable two-factor authentication now'
          : 'Thanks for using modern security practices';
      ...
      return next(
        wrapError(
          error,
          `We were unable to validate your security settings with GitHub. The error GitHub returned: ${
            error.message || error
          }`
        )
      );
