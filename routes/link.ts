...
router.use(
  asyncHandler(async (req: IRequestHacked, res, next) => {
    ...
    try {
      ...
      if (
        config &&
        config.activeDirectoryGuests &&
        config.activeDirectoryGuests.authorizedIds &&
        config.activeDirectoryGuests.authorizedIds.length &&
        config.activeDirectoryGuests.authorizedIds.includes(aadId)
      ) {
        block = false;
        blockedRecord =
